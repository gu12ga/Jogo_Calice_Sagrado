...
public class PalavrasComando {
    ...
    private static final String[] comandosValidos = {
        "ir", "sair", "ajuda", "observar", "pegar", "usar", "comer", "desequipar", "largar"
    };

    private static final String[] comandosBatalha = {
        "atacar", "escapar"
    };

    public String getComandos(){
        String cmd = "";
        for(String s : comandosValidos)
            cmd += s + " ";
        return cmd;
    }
    ...
    public boolean ehComando(String umaString) {
        ...
        for(int i = 0; i < comandosBatalha.length; i++){
            if(comandosBatalha[i].equals(umaString))
                return true;
        }
